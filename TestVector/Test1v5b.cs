...
namespace SIMDeefect
{
    ...
    {
        public unsafe static uint VectorSum5b(ReadOnlySpan<char> charSpan, int offset, int length, bool alignVector)
        {
            ...
            fixed(char* fixedPtr = charSpan)
            {
                ...
                for (i = blockCount; 0 < i; i--)
                {
                    ...
                    areEqualA = Vector.Equals(vector2, maskA);
                    accResultA = Vector.Subtract(accResultA, areEqualA);

                    areEqualE = Vector.Equals(vector2, maskE);
                    accResultE = Vector.Subtract(accResultE, areEqualE);

                    areEqualI = Vector.Equals(vector2, maskI);
                    accResultI = Vector.Subtract(accResultI, areEqualI);

                    areEqualO = Vector.Equals(vector2, maskO);
                    accResultO = Vector.Subtract(accResultO, areEqualO);

                    areEqualU = Vector.Equals(vector2, maskU);
                    accResultU = Vector.Subtract(accResultU, areEqualU);

                    // vector3  ------------------------------------------------------------------------
                    
                    areEqualA = Vector.Equals(vector3, maskA);
                    accResultA = Vector.Subtract(accResultA, areEqualA);

                    areEqualE = Vector.Equals(vector3, maskE);
                    accResultE = Vector.Subtract(accResultE, areEqualE);

                    areEqualI = Vector.Equals(vector3, maskI);
                    accResultI = Vector.Subtract(accResultI, areEqualI);

                    areEqualO = Vector.Equals(vector3, maskO);
                    accResultO = Vector.Subtract(accResultO, areEqualO);

                    areEqualU = Vector.Equals(vector3, maskU);
                    accResultU = Vector.Subtract(accResultU, areEqualU);
        ...
        internal static uint Test1vector5b(string S, int offset, int length, bool alignVector)
        {
            //                                                               ~0,14
            var counts = new uint[64 * 1024];
            var len = S.Length;

            var result = VectorSum5b(S, offset, length, alignVector);
